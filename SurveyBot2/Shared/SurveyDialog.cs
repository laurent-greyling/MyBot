...
using SurveyBot2.Models;
using HtmlDocument = HtmlAgilityPack.HtmlDocument;

namespace SurveyBot2.Shared
{
    [Serializable]
    public class SurveyDialog : IDialog<object>
    {
        ...
        public async Task StartAsync(IDialogContext context)
        {
            ...
            //This should make the interview identified (Add a respondent key to end of interview link)
            var idUserLink = $"{LiveLink}/{context.Activity.Id}{new Random().Next(10, 100)}";
            var response = await client.GetAsync(idUserLink);
        ...
        public async Task MessageReceivedAsync(IDialogContext context, IAwaitable<IMessageActivity> argument)
        {
            ...
            if (!string.IsNullOrWhiteSpace(foundAnswer))
            {
                ...
                {
                    await context.PostAsync("Thank you, seems we are done");
                    context.Done("Enjoy the rest of your day");
                }
                else
                {
                    context.UserData.SetValue("screen", capturedScreen);
